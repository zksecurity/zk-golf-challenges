...
/-!
Public interface for the BLS12-381 G1 fixed-base scalar-multiplication
instance.
...
The base point is fixed to the standard generator `G` (not part of the
input), so the only input is the scalar as a bit sequence (most significant
first). The generator lies on the curve and in the prime-order subgroup, so
no base-point assumptions are needed; and since the base point is fixed and
known to generate the prime-order subgroup, the output `[k]G` automatically
lies in that subgroup as well — the spec therefore does not require the
circuit to witness subgroup membership (unlike the variable-base
instance).

Because the base point is fixed, the circuit has room to be far cheaper
than the variable-base version (e.g. precomputed multiples of `G`).
...
namespace Challenge.Instances.Bls12381G1ScalarMulFixedBase
...
/--
  ...
  which lies on the curve and in the prime-order subgroup, so no base-point
...
/--
  ...
  (`Specs.Bls12381G1ScalarMulFixedBase.Spec`, naive double-and-add over
  the bits with base point `G` and the complete group law). No
  subgroup-membership obligation: the fixed base point is known to
  generate the prime-order subgroup, so the true multiple lies there
  automatically.
-/
def Spec (input : Input (F circomPrime)) (output : Output (F circomPrime))
    (_data : ProverData (F circomPrime)) : Prop :=
  OutputValid output ∧
  Specs.Bls12381G1ScalarMulFixedBase.Spec (input.bits.map ZMod.val)
    (decodeOutput output)
...
end Challenge.Instances.Bls12381G1ScalarMulFixedBase
