...
namespace Challenge.Instances.KeccakP800GF2

namespace Interface

open Challenge.F2Bits

@[reducible] def permutationBits : ℕ := Specs.KeccakP800.stateBits
...
def Spec (input : Input (F p2)) (output : Output (F p2)) (_data : ProverData (F p2)) :
    Prop :=
  output.state = Specs.KeccakP800.keccakP800_12 input.state
...
end Challenge.Instances.KeccakP800GF2
