...
def upperSigma1 (x : Var (fields 32) (F p)) : Circuit (F p) (Var (fields 32) (F p)) := do
  let r1 ← Xor32.circuit ⟨rotr32 6 x, rotr32 11 x⟩
  Xor32.circuit ⟨r1, rotr32 25 x⟩
...
theorem soundness : Soundness (F p) main Assumptions Spec := by
  circuit_proof_start [upperSigma1, Xor32.circuit]
  ...
  obtain ⟨v1, n1⟩ := c1 nr6 nr11
  obtain ⟨v2, n2⟩ := c2 n1 nr25
  refine ⟨?_, n2⟩
  rw [v2, v1, valueBits_eval_rotr32 env input_var input h_input h_assumptions 6,
...
theorem completeness : Completeness (F p) main Assumptions := by
  ...
  obtain ⟨_, n1⟩ := h_env.1 nr6 nr11
  exact ⟨⟨nr6, nr11⟩, n1, nr25⟩
...
theorem computableWitnesses : (circuit (p := p)).ComputableWitnesses := by
  ...
  unfold main upperSigma1
  let firstInput : Var Xor32.Inputs (F p) := ⟨rotr32 6 input, rotr32 11 input⟩
  ...
  · exact Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
      Xor32.circuit input ⟨r1, rotr32 25 input⟩ n1
      (by
