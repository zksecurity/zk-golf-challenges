...
def main (v : Var (fields 512) (F p2)) : Circuit (F p2) (Var (fields 512) (F p2)) := do
  let w ← Circuit.foldlRange 16
    ...
    (fun w i => do
      let ui ← ScheduleStep.circuit (c128 (w[i.val]'(by omega)) (w[i.val + 1]'(by omega))
  ...
  elaborate_circuit_with {
    ...
    · have hfold : (Fin.foldl 16
          (fun (acc : Vector (fields 32 (Expression (F p2))) 32) (i : Fin 16) =>
            acc.set (16 + i.val)
              (Vector.mapRange 32 fun i_1 => var { index := n + i.val * 125 + 31 + 31 + 31 + i_1 })
              (by omega))
...
theorem soundness : Soundness (F p2) main Assumptions Spec := by
  circuit_proof_start [main, Spec, ScheduleStep.Assumptions, ScheduleStep.Spec]
  ...
  have Q : ∀ k, k ≤ 16 → ∀ j (hj : j < 32),
    ...
    | succ k ih =>
      ...
      replace hstep := hstep (by simp only [ScheduleStep.circuit, ScheduleStep.Assumptions])
      rw [show ScheduleStep.circuit.Spec = ScheduleStep.Spec from rfl] at hstep
      simp only [ScheduleStep.Spec, wordAt_eval_c128_0, wordAt_eval_c128_1, wordAt_eval_c128_2,
...
theorem completeness : Completeness (F p2) main Assumptions := by
  circuit_proof_start
  intro i
  simp only [ScheduleStep.circuit, ScheduleStep.Assumptions]
...
theorem subcircuit_localLength (v : Var (fields 512) (F p2)) (m : ℕ) :
    (subcircuit circuit v).localLength m = 2000 := rfl

/-- The chunk's output is the 16 witnessed step words. -/
theorem eval_subOut_of_agreesBelow (v : Var (fields 512) (F p2)) (n : ℕ) {kk : ℕ}
    (hk : n + 2000 ≤ kk) {env env' : ProverEnvironment (F p2)}
...
-- Keep the unifier from expanding the 125-witness step body while peeling the
...
theorem computableWitnesses : circuit.ComputableWitnesses := by
  ...
  simp only [
    ...
    ScheduleStep.subcircuit_localLength, and_true]
  intro i
  obtain ⟨iv, hiv⟩ := i
  ...
  refine FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
    ScheduleStep.circuit input _ _ ?_ ScheduleStep.computableWitnesses env env'
  intro kk e e' hle h_agree h_input
  -- the loop offset is `offset + iv * 125` (`stepBody_localLength`, definitionally)
  have hle' : offset + iv * 125 ≤ kk := hle
