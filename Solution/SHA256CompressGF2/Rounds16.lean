...
def main (r0 : ℕ) (v : Var (fields 768) (F p2)) : Circuit (F p2) (Var (fields 256) (F p2)) := do
  let s16 ← Circuit.foldlRange 16 (st768 v) (fun s i =>
    Round.circuit (kAt (r0 + i.val)) (c288s s (q768 v (8 + i.val)))) (constantLength r0 v)
  Pin256.circuit s16
...
instance elaborated (r0 : ℕ) :
  ...
  elaborate_circuit_with {
    localLength _ := 5776
    output _ i₀ := varFromOffset (fields 256) (i₀ + 5520)
  } using by
...
theorem soundness (r0 : ℕ) : Soundness (F p2) (main r0) Assumptions (Spec r0) := by
  circuit_proof_start [main, Spec, Round.Assumptions, Round.Spec,
  ...
  have hLL : ∀ (k : ℕ) (x : Var (fields 288) (F p2)),
      (Round.circuit k).localLength x = 345 := by
    intro k x; simp [circuit_norm, Round.circuit, Round.elaborated]
  ...
  -- Inductive invariant: the words of the accumulator after `k` rounds are `applyN … k`.
  have Q : ∀ k, k ≤ 16 →
    ...
    | succ k ih =>
      ...
      replace hstep := hstep (by simp only [Round.circuit, Round.Assumptions])
      rw [show (Round.circuit (kAt (r0 + k))).Spec = Round.Spec (kAt (r0 + k)) from rfl] at hstep
      simp only [Round.Spec] at hstep
      ...
      rw [stateVar256]
      exact hstep
  simp only [Pin256.circuit, Pin256.elaborated, Pin256.Assumptions, Pin256.Spec, true_implies] at hpin
  ...
  rw [show (Vector.map (Expression.eval env) (Vector.mapRange 256 fun i => var { index := i₀ + 5520 + i }))
...
theorem completeness (r0 : ℕ) : Completeness (F p2) (main r0) Assumptions := by
  circuit_proof_start
  simp [Round.circuit, Round.Assumptions, Pin256.circuit, Pin256.Assumptions]
...
-- Keep the unifier from expanding the 345-witness round body while peeling the
-- loop; the offsets and outputs are supplied by the `rfl` lemmas instead.
attribute [local irreducible] Round.circuit Pin256.circuit main
...
theorem computableWitnesses (r0 : ℕ) : (circuit r0).ComputableWitnesses := by
  ...
  simp only [
    ...
    Round.subcircuit_localLength, Circuit.foldlRange.localLength_eq,
  ...
  · intro i
    ...
    refine FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
      (Round.circuit (kAt (r0 + iv))) input _ _ ?_ (Round.computableWitnesses _) env env'
  ...
  · rw [fin_foldl_subcircuit_eq_stateVar256 r0 offset input 16]
    refine FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
      Pin256.circuit input _ _ ?_ Pin256.computableWitnesses env env'
    intro kk e e' hle h_agree h_input
...
theorem subcircuit_localLength (r0 : ℕ) (v : Var (fields 768) (F p2)) (m : ℕ) :
    (subcircuit (circuit r0) v).localLength m = 5776 := rfl

/-- The chunk's output is the trailing `Pin256` witness block at relative offset 5520. -/
theorem eval_subOut_of_agreesBelow (r0 : ℕ) (v : Var (fields 768) (F p2)) (n : ℕ) {kk : ℕ}
    (hk : n + 5776 ≤ kk) {env env' : ProverEnvironment (F p2)}
    (h_agree : env.AgreesBelow kk env') :
    eval env ((subcircuit (circuit r0) v).output n)
      = eval env' ((subcircuit (circuit r0) v).output n) := by
  have hout : (subcircuit (circuit r0) v).output n = varFromOffset (fields 256) (n + 5520) := rfl
  rw [hout]
