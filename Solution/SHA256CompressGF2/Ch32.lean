...
/-- Input `v = e ‖ f ‖ g`. Witness `ch_j = g_j + e_j·(f_j + g_j)`, pin with
`(w − g) − e·(f + g) = 0`.
...
def main (v : Var (fields 96) (F p2)) : Circuit (F p2) (Var (fields 32) (F p2)) := do
  let w ← Circuit.witnessVector 32 (.lit <| .ofFn fun i : Fin 32 => Witgen.FExpr.expr
    (a96 v (64 + i.val) + a96 v i.val * (a96 v (32 + i.val) + a96 v (64 + i.val))))
  Circuit.forEach (Vector.finRange 32) (fun i =>
    assertZero ((w[i.val]'i.isLt - a96 v (64 + i.val))
      - a96 v i.val * (a96 v (32 + i.val) + a96 v (64 + i.val))))
  return w
...
/-- `Ch` on the packed words of `v = e ‖ f ‖ g`. -/
def Spec (v : fields 96 (F p2)) (out : fields 32 (F p2)) : Prop :=
  toNat out = Specs.SHA256.Ch (wordAt 32 v 0) (wordAt 32 v 1) (wordAt 32 v 2)
...
theorem soundness : Soundness (F p2) main Assumptions Spec := by
  ...
  have hv : ∀ (m : ℕ) (hm : m < 96),
    ...
    exact h2
  refine ch_words input _ fun j hj => ?_
...
theorem computableWitnesses : circuit.ComputableWitnesses := by
  ...
  · intro _ h_input
    ...
    -- the witnessed cell is the literal `Ch` expression, so it reads only the input
    simp only [a96, circuit_norm]
    rw [eval_getElem_congr h_input, eval_getElem_congr h_input, eval_getElem_congr h_input]
  · intro _
    trivial
