...
The carries are `Add32.carriesIR` (the closed-form binary ripple carry); the products
reuse the same closed-form carry `Add32.carryIR` at the running index, so this gadget
adds no new recursion to the IR. -/
