...
It is written from circuit primitives only (`Circuit.witnessVector`, `Circuit.forEach`,
...
The bits are generated by the witness IR (`VExpr.bitsOf`, the exact field-element
bit decomposition — not a u64 shift, which would truncate a wide field value),
which `completeness` reads back as `fieldToBits`.
...
def main (n : ℕ) (x : Expression (F circomPrime)) : Circuit (F circomPrime) Unit := do
  let bits ← Circuit.witnessVector n (x.bits n)
...
theorem soundness (n : ℕ) :
  ...
  have hin : input = fieldFromBits (bit_vars.map env) := by
    rw [sub_eq_zero.mp h_eq]; exact fieldFromBits_eval bit_vars
  rw [hin]
...
theorem completeness (n : ℕ) :
  ...
  circuit_proof_start [main, Spec]
  -- the shift-and-mask witness program computes exactly `fieldToBits input`
  replace h_env : ∀ i : Fin n, env.get (i₀ + i.val) = (fieldToBits n input)[i.val] := by
    intro i
    rw [h_env i, getElem_fieldToBits, ← h_input]
  set bit_vars : Vector (Expression (F circomPrime)) n :=
    Vector.mapRange n (fun i => var ⟨i₀ + i⟩) with hbv
  refine ⟨?_, ?_⟩
