import Solution.Bls12381G1ScalarMulFixedBase.CompleteAdd
...
def Spec (input : Inputs (F circomPrime)) (out : FlaggedPoint (F circomPrime)) : Prop :=
  out.Valid ∧
    decodePoint out =
      Specs.ShortWeierstrass.step Specs.Bls12381G1.curve
        { x := decodeFe input.px, y := decodeFe input.py }
...
(`doubled` at `offset`, `added` at `offset + 24543`, the mux output at
`offset + 49086`). -/
...
private lemma completeAdd_output_stable (X : Var CompleteAdd.Inputs (F circomPrime))
    ...
    (h_agree : env.AgreesBelow k env') (hk : o + 24543 ≤ k) :
...
theorem computableWitnesses : circuit.ComputableWitnesses := by
  ...
  have hca : ∀ (X : Var CompleteAdd.Inputs (F circomPrime)) (o : ℕ),
      (subcircuit CompleteAdd.circuit X).localLength o = 24543 := fun _ _ => rfl
  ...
  · refine Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
    ...
    · -- ifTrue = added (CompleteAdd output at offset + 24543)
      have ha := completeAdd_output_stable { P := doubled, Q := base } (o := offset + 24543)
        h_agree (by omega)
...
selected point), allocated at `offset + 49086` (after the two `CompleteAdd`
blocks) and reading only its `size FlaggedPoint = 13` cells. Environments agreeing
below any `k ≥ offset + 49099` (the full local length) evaluate the output
identically. Consumed by `ScalarMul`. -/
lemma eval_output_of_agreesBelow (input : Var Inputs (F circomPrime)) {offset k : ℕ}
    {env env' : ProverEnvironment (F circomPrime)}
    (h_agree : env.AgreesBelow k env') (hk : offset + 49099 ≤ k) :
  ...
  exact fpVar_stable (off := offset + 49086) h_agree (by rw [hsz]; omega)
