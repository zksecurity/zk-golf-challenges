...
/-- A canonical (`< P381`) `emuOfNat` is a valid emulated field element. -/
lemma fe_valid_emuOfNat {v : ℕ} (hv : v < P381) : Fe.Valid (emuOfNat v) :=
  ⟨emuOfNat_normalized v, by rw [value_emuOfNat (lt_trans hv P381_lt)]; exact hv⟩
...
lemma fe_valid_eval_zeroConst (env : Environment (F circomPrime)) :
  ...
  exact fe_valid_emuOfNat P381_pos

/-! ## Decoded corollaries of the subcircuit specs -/

/-- The decoded `MulMod` output (with modulus `P381`) is the base-field
product. -/
lemma decodeFe_of_mulmod {a b out : Emu (F circomPrime)}
    (hout : BigInt.value limbBits out
      = BigInt.value limbBits a * BigInt.value limbBits b % P381) :
    decodeFe out = decodeFe a * decodeFe b := by
  rw [decodeFe, decodeFe, decodeFe, hout, ZMod.natCast_mod, Nat.cast_mul]

/-- A normalized output reduced mod `P381` is a valid emulated field
element. -/
lemma fe_valid_of_mod {x : Emu (F circomPrime)} {v : ℕ}
    (hnorm : x.Normalized limbBits)
    (hval : BigInt.value limbBits x = v % P381) : Fe.Valid x :=
  ⟨hnorm, hval ▸ Nat.mod_lt _ P381_pos⟩

/-! ## Field dichotomies -/

/-- `2 ≠ 0` in the secp256k1 base field (its characteristic is a 256-bit
prime). -/
lemma two_ne_zero_fp : (2 : Specs.Bls12381G1.Fp) ≠ 0 := by
  have h2 : ((2 : ℕ) : Specs.Bls12381G1.Fp) ≠ 0 := by
    rw [Ne, ZMod.natCast_eq_zero_iff]
    intro hdvd
    have hle : P381 ≤ 2 := Nat.le_of_dvd (by norm_num) hdvd
    have : 2 < P381 := by decide
    omega
  simpa using h2
...
lemma add_inf_left (q : GroupPoint Specs.Bls12381G1.Fp) :
    add Specs.Bls12381G1.curve .infinity q = q := rfl

open Specs.ShortWeierstrass in
lemma add_inf_right (p : Point Specs.Bls12381G1.Fp) :
    add Specs.Bls12381G1.curve (.affine p) .infinity = .affine p := rfl

open Specs.ShortWeierstrass in
lemma add_affine (px py qx qy : Specs.Bls12381G1.Fp) :
    add Specs.Bls12381G1.curve (.affine ⟨px, py⟩) (.affine ⟨qx, qy⟩)
      = if px = qx then
          (if py = -qy then .infinity
            else .affine (tangent Specs.Bls12381G1.curve ⟨px, py⟩))
        else .affine (chord ⟨px, py⟩ ⟨qx, qy⟩) := rfl

open Specs.ShortWeierstrass in
/-- The BLS12-381 curve equation, with the concrete coefficients inlined. -/
lemma onCurve_iff (p : Point Specs.Bls12381G1.Fp) :
    OnCurve Specs.Bls12381G1.curve p ↔ p.y ^ 2 = p.x ^ 3 + 4 := by
  simp [OnCurve, Specs.Bls12381G1.curve]

open Specs.ShortWeierstrass in
/-- The secp256k1 tangent formula, with `a = 0` inlined. -/
lemma tangent_eq (px py : Specs.Bls12381G1.Fp) :
    tangent Specs.Bls12381G1.curve ⟨px, py⟩
      = { x := (3 * px ^ 2 / (2 * py)) ^ 2 - 2 * px,
          y := 3 * px ^ 2 / (2 * py)
            * (px - ((3 * px ^ 2 / (2 * py)) ^ 2 - 2 * px)) - py } := by
  simp [tangent, Specs.Bls12381G1.curve]

open Specs.ShortWeierstrass in
/-- The chord formula, componentwise. -/
lemma chord_eq (px py qx qy : Specs.Bls12381G1.Fp) :
    chord ⟨px, py⟩ ⟨qx, qy⟩
...
theorem soundness_core
    ...
    out.Valid ∧
      decodePoint out =
        Specs.ShortWeierstrass.add Specs.Bls12381G1.curve (decodePoint P) (decodePoint Q) := by
  ...
  · -- P is finite
    ...
    · -- Q is finite too: the generic affine case analysis
      ...
      rw [hs2, if_neg hQne1, decodePoint_of_finite hPinf, decodePoint_of_finite hQinf,
        add_affine]
      have hPc : decodeFe P.y ^ 2 = decodeFe P.x ^ 3 + 4 :=
        (onCurve_iff _).mp (hP.2.2.2 hPinf)
      have hQc : decodeFe Q.y ^ 2 = decodeFe Q.x ^ 3 + 4 :=
