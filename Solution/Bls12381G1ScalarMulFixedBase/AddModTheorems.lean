import Solution.Bls12381G1ScalarMulFixedBase.Params
...
lemma P381_pos : 0 < P381 := by decide

/-- The emulated prime fits in the `numLimbs · limbBits = 256` available bits. -/
lemma P381_lt : P381 < 2 ^ (limbBits * numLimbs) := by decide
...
lemma limb_sum_P381 :
    (∑ k ∈ Finset.range numLimbs, limbOfNat P381 k * 2 ^ (limbBits * k)) = P381 := by
  have h : (∑ k ∈ Finset.range numLimbs, limbOfNat P381 k * 2 ^ (limbBits * k))
      = ∑ k ∈ Finset.range numLimbs,
          (P381 / 2 ^ (limbBits * k) % 2 ^ limbBits) * 2 ^ (limbBits * k) :=
    Finset.sum_congr rfl fun k _ => rfl
  rw [h, limb_decomp_mod, Nat.mod_eq_of_lt P381_lt]

/-! ## `pConst` evaluation facts -/

lemma eval_pConst_getElem (env : Environment (F circomPrime)) (k : ℕ) (hk : k < numLimbs) :
    Expression.eval env (pConst[k]'hk) = ((limbOfNat P381 k : ℕ) : F circomPrime) :=
  congrArg (Expression.eval env) (Vector.getElem_ofFn ..)

lemma eval_pConst (env : Environment (F circomPrime)) :
    Vector.map (Expression.eval env) pConst = emuOfNat P381 := by
  apply Vector.ext
  intro k hk
  rw [Vector.getElem_map, eval_pConst_getElem env k hk, emuOfNat_getElem P381 k hk]

lemma pConst_normalized (env : Environment (F circomPrime)) :
    BigInt.Normalized limbBits (Vector.map (Expression.eval env) pConst) := by
  rw [eval_pConst]
  exact emuOfNat_normalized P381

lemma pConst_value (env : Environment (F circomPrime)) :
    BigInt.value limbBits (Vector.map (Expression.eval env) pConst) = P381 := by
  rw [eval_pConst]
  exact value_emuOfNat P381_lt
...
lemma val_q_mul_limb {qN : ℕ} (hqN : qN ≤ 1) (k : ℕ) :
    (((qN : ℕ) : F circomPrime) * ((limbOfNat P381 k : ℕ) : F circomPrime)).val
      = qN * limbOfNat P381 k := by
  ...
  have h1 := limbOfNat_lt P381 k
  have h2 : qN * limbOfNat P381 k ≤ 1 * limbOfNat P381 k :=
...
lemma rhs_bounds (env : Environment (F circomPrime)) (i₀ : ℕ) (qN : ℕ)
  ...
  · rw [dif_pos hk,
    ...
    have h1 := ZMod.val_add_le
      (((qN : ℕ) : F circomPrime) * ((limbOfNat P381 k.val : ℕ) : F circomPrime))
      (env.get (i₀ + k.val))
    rw [val_q_mul_limb hqN k.val] at h1
    have h2 := limbOfNat_lt P381 k.val
    have h3 : qN * limbOfNat P381 k.val ≤ 1 * limbOfNat P381 k.val :=
      Nat.mul_le_mul_right _ hqN
    have h4 := hr k.val hk
    have := limb_add_le_bound
    omega
...
lemma polyValue_rhs (env : Environment (F circomPrime)) (i₀ : ℕ) (qN : ℕ)
    ...
    polyValue limbBits (Vector.map (Expression.eval env)
      ...
      = qN * P381 + BigInt.value limbBits (Vector.map (Expression.eval env)
          (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) := by
  refine (polyValue_padded limbBits env
      (fun k h => var { index := i₀ + numLimbs } * pConst[k.val]'h + var { index := i₀ + k.val })
      (fun k => qN * limbOfNat P381 k + (env.get (i₀ + k)).val) ?_).trans ?_
  · intro k hk
  ...
  · rw [value_outVar limbBits env i₀]
    have hsplit : (∑ k ∈ Finset.range numLimbs,
          (qN * limbOfNat P381 k + (env.get (i₀ + k)).val) * 2 ^ (limbBits * k))
        = (∑ k ∈ Finset.range numLimbs, qN * (limbOfNat P381 k * 2 ^ (limbBits * k)))
          + ∑ k ∈ Finset.range numLimbs, (env.get (i₀ + k)).val * 2 ^ (limbBits * k) := by
      rw [← Finset.sum_add_distrib]
      exact Finset.sum_congr rfl fun k _ => by ring
    rw [hsplit, ← Finset.mul_sum, limb_sum_P381]
...
lemma soundness_core (i₀ : ℕ) (env : Environment (F circomPrime))
  ...
  have hr_lt : BigInt.value limbBits (Vector.map (Expression.eval env)
      (Vector.mapRange numLimbs fun i => var { index := i₀ + i })) < P381 := by
    have h := h_lt_impl ⟨hr_norm, pConst_normalized env⟩
    rwa [pConst_value env] at h
  refine ⟨⟨hr_norm, hr_lt⟩, ?_⟩
  -- push the ℕ identity `a + b = q·P381 + r` into the emulated field
  have hcast := congrArg (Nat.cast : ℕ → Specs.Bls12381G1.Fp) h_polyeq
  push_cast at hcast
  rw [show ((P381 : ℕ) : Specs.Bls12381G1.Fp) = 0 from ZMod.natCast_self _,
    mul_zero, zero_add] at hcast
  simp only [decodeFe]
  exact hcast.symm

/-- **Completeness core.** The honest witnesses (`r = (a + b) % P381` limbs,
`q = (a + b) / P381`) satisfy every constraint of `AddMod.main`. -/
lemma completeness_core (i₀ : ℕ) (env : Environment (F circomPrime))
    ...
    (h_wit_r : ∀ i : Fin numLimbs,
      env.get (i₀ + i.val)
        = (emuOfNat ((BigInt.value limbBits a + BigInt.value limbBits b) % P381))[i.val])
    (h_wit_q : env.get (i₀ + numLimbs)
      = (((BigInt.value limbBits a + BigInt.value limbBits b) / P381 : ℕ) : F circomPrime)) :
  ...
  -- the quotient is boolean since a + b < 2·P381
  have hq2 : (va + vb) / P381 ≤ 1 := by
    have h := (Nat.div_lt_iff_lt_mul P381_pos).mpr (by omega : va + vb < 2 * P381)
    omega
  set qNat := (va + vb) / P381 with hqNat
  set rN := (va + vb) % P381 with hrN
  have hrN_lt : rN < P381 := Nat.mod_lt _ P381_pos
  have hrN_pow : rN < 2 ^ (limbBits * numLimbs) := lt_trans hrN_lt P381_lt
  -- the witnessed output limbs are the canonical digits of (a + b) % P381
  ...
  · -- the EqViaCarries identity: a + b = q·P381 + r over ℕ
    rw [polyValue_lhs env a_var b_var a b h_input_a h_input_b ha_norm hb_norm,
      polyValue_rhs env i₀ qNat h_wit_q hq2 (outVar_val_lt env i₀ hrv_norm),
      hrv_val]
    calc va + vb = P381 * qNat + rN := by
          rw [hqNat, hrN]
          exact (Nat.div_add_mod (va + vb) P381).symm
      _ = qNat * P381 + rN := by ring
