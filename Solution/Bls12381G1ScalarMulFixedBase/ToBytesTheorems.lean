...
theorem soundness_core (env : Environment (F circomPrime)) (i₀ : ℕ)
  ...
  -- regroup the 48-byte base-2^8 sum into the 6-limb base-2^64 sum
  calc (∑ i : Fin coordBytes,
        ((Vector.map (Expression.eval env)
          (Vector.mapRange coordBytes fun j =>
            var (F := F circomPrime) { index := i₀ + j }))[i]).val * 2 ^ (8 * i.val))
      = ∑ i ∈ Finset.range (6 * 8), (env.get (i₀ + i)).val * 2 ^ (8 * i) := by
        rw [← Fin.sum_univ_eq_sum_range (fun i => (env.get (i₀ + i)).val * 2 ^ (8 * i)) 48]
        apply Finset.sum_congr rfl
        intro i _
    ...
    _ = ∑ k ∈ Finset.range 6,
          (∑ t ∈ Finset.range 8, (env.get (i₀ + (8 * k + t))).val * 2 ^ (8 * t))
            * 2 ^ (8 * 8 * k) :=
        sum_regroup 8 8 (fun i => (env.get (i₀ + i)).val) 6
