import Solution.Bls12381G1ScalarMulFixedBase.AddMod
import Solution.Bls12381G1ScalarMulFixedBase.SubMod
import Solution.Bls12381G1ScalarMulFixedBase.DivOrZero
import Solution.Bls12381G1ScalarMulFixedBase.CompleteAddTheorems
...
def Spec (input : Inputs (F circomPrime)) (out : FlaggedPoint (F circomPrime)) : Prop :=
  out.Valid ∧
    decodePoint out =
      Specs.ShortWeierstrass.add Specs.Bls12381G1.curve
...
theorem soundness : Soundness (F circomPrime) main Assumptions Spec := by
  ...
  obtain ⟨hx1sqn, hx1sqe⟩ := hx1sq ⟨hPx.1, hPx.1, hpn,
    by rw [hpv]; exact hPx.2, by rw [hpv]; exact hPx.2, by rw [hpv]; exact P381_pos⟩
  rw [hpv] at hx1sqe
  ...
  obtain ⟨hlamSqn, hlamSqe⟩ := hlamSq ⟨hlamv.1, hlamv.1, hpn,
    by rw [hpv]; exact hlamv.2, by rw [hpv]; exact hlamv.2, by rw [hpv]; exact P381_pos⟩
  rw [hpv] at hlamSqe
  ...
  obtain ⟨hyprodn, hyprode⟩ := hyprod ⟨hlamv.1, hxdv.1, hpn,
    by rw [hpv]; exact hlamv.2, by rw [hpv]; exact hxdv.2, by rw [hpv]; exact P381_pos⟩
  rw [hpv] at hyprode
...
theorem computableWitnesses : circuit.ComputableWitnesses := by
  ...
  have hsub : ∀ (X : Var SubMod.Inputs (F circomPrime)) (o : ℕ),
      (subcircuit SubMod.circuit X).localLength o = 1557 := fun _ _ => rfl
  have hadd : ∀ (X : Var AddMod.Inputs (F circomPrime)) (o : ℕ),
      (subcircuit AddMod.circuit X).localLength o = 1557 := fun _ _ => rfl
  have hmul : ∀ (X : Var (MulMod.Inputs numLimbs) (F circomPrime)) (o : ℕ),
      (subcircuit (MulMod.circuit secpParams) X).localLength o = 2018 := fun _ _ => rfl
  have hdiv : ∀ (X : Var DivOrZero.Inputs (F circomPrime)) (o : ℕ),
      (subcircuit DivOrZero.circuit X).localLength o = 2833 := fun _ _ => rfl
  have hmxE : ∀ (X : Var (Mux.Inputs Emu) (F circomPrime)) (o : ℕ),
      (subcircuit (Mux.circuit (M := Emu)) X).localLength o = 6 := fun _ _ => rfl
  have hmxF : ∀ (X : Var (Mux.Inputs FlaggedPoint) (F circomPrime)) (o : ℕ),
      (subcircuit (Mux.circuit (M := FlaggedPoint)) X).localLength o = 13 := fun _ _ => rfl
  have hisz : ∀ (x : Var Emu (F circomPrime)) (o : ℕ),
      (subcircuit IsZeroFe.circuit x).localLength o = 17 := fun _ _ => rfl
  ...
  have iszOut : ∀ (x : Var Emu (F circomPrime)) {e e' : ProverEnvironment (F circomPrime)}
      {O k' : ℕ}, e.AgreesBelow k' e' → O + 17 ≤ k' →
  ...
  have hnl : numLimbs = 6 := rfl
  have hsE : size Emu = 6 := rfl
  have hsF : size FlaggedPoint = 13 := rfl
  ...
  let s1 : Var FlaggedPoint (F circomPrime) :=
    (subcircuit (Mux.circuit (M := FlaggedPoint))
      ...
      (offset + 1557 + 1557 + 17 + 1557 + 17 + 2018 + 1557 + 1557 + 1557 + 6 + 6 + 2833 + 2018 + 1557 + 1557 + 1557 + 2018 + 1557 + 1)
  unfold main
  simp only [
  ...
  · refine Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
      (Parent := Inputs) IsZeroFe.circuit _
      ((subcircuit AddMod.circuit { a := P.y, b := Q.y }).output (offset + 1557 + 1557 + 17)) _ ?_
      IsZeroFe.computableWitnesses env env'
    intro k e e' hle h_agree _h_in
  ...
  · simp only [HasAssignEq.assignEq,
    ...
    · -- the witnessed cancellation flag `sameX * oppY` is stable
      ...
      exact mulStable _ _
        (iszOut dx (O := offset + 1557 + 1557) h_agree (by omega))
        (iszOut sy (O := offset + 1557 + 1557 + 17 + 1557) h_agree (by omega))
  ...
  · refine Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
    ...
    · -- selector = cancel, a fresh witness cell
      exact assignEq_output_eval_stable (sameX * oppY)
        (base := offset + 1557 + 1557 + 17 + 1557 + 17 + 2018 + 1557 + 1557 + 1557 + 6 + 6 + 2833
          + 2018 + 1557 + 1557 + 1557 + 2018 + 1557) h_agree (by omega)
  ...
  · refine Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
    ...
    · -- ifFalse = s1, a fresh `Mux` witness block
      have hs1 := Mux.eval_output_of_agreesBelow (M := FlaggedPoint)
        { selector := cancel, ifTrue := infConst, ifFalse := finite }
        (offset := offset + 1557 + 1557 + 17 + 1557 + 17 + 2018 + 1557 + 1557 + 1557 + 6 + 6 + 2833
          + 2018 + 1557 + 1557 + 1557 + 2018 + 1557 + 1) h_agree (by omega)
  ...
  · refine Challenge.Utils.ComputableWitnessLemmas.FormalCircuit.subcircuit_flatStructuralComputableWitnesses_of_condition
    ...
    · -- ifFalse = s2, a fresh `Mux` witness block
      have hs2 := Mux.eval_output_of_agreesBelow (M := FlaggedPoint)
        { selector := Q.isInf, ifTrue := P, ifFalse := s1 }
        (offset := offset + 1557 + 1557 + 17 + 1557 + 17 + 2018 + 1557 + 1557 + 1557 + 6 + 6 + 2833
          + 2018 + 1557 + 1557 + 1557 + 2018 + 1557 + 1 + 13) h_agree (by omega)
...
point `out`), allocated at `offset + 24530` and reading only its `size FlaggedPoint = 13`
cells. Environments agreeing below any `k ≥ offset + 24543` (the full local length)
...
lemma eval_output_of_agreesBelow (input : Var Inputs (F circomPrime)) {offset k : ℕ}
    {env env' : ProverEnvironment (F circomPrime)}
    (h_agree : env.AgreesBelow k env') (hk : offset + 24543 ≤ k) :
  ...
  exact fpVar_stable (off := offset + 24530) h_agree (by rw [hsz]; omega)
