import Solution.Bls12381G1ScalarMulFixedBase.SubModTheorems
...
Witness the reduced difference `r = (a + P381 − b) % P381` and the borrow bit
`q` characterized by the integer identity

  `r + b = a + q · P381`   (`q = 0` when `b ≤ a`, `q = 1` otherwise),
...
`r = (a + P381 − b) % P381` is a subtraction of 256-bit values, which the digit layer
does directly: the register starts at `a + P381` (a shift large enough that the
subtraction of `b` never borrows), `b` is subtracted, and one conditional reduction by
the modulus brings the result back below `P381`. The borrow of *that* reduction is the
...
def main (input : Var Inputs (F circomPrime)) :
  ...
  -- witness r = (a + P381 - b) % P381 and the borrow bit, out of the same register
  ...
  -- r + b = a + q·P381 as integers, limb-coefficient-wise
...
theorem completeness : Completeness (F circomPrime) main Assumptions := by
  ...
  have halt : evalEmu env input_var_a < P381 := by
    rw [heva]; exact h_assumptions.1.2
  have hblt : evalEmu env input_var_b < P381 := by
  ...
  have hwit_r : ∀ i : Fin numLimbs, env.toEnvironment.get (i₀ + i.val)
      = (emuOfNat ((BigInt.value limbBits input_a + P381 - BigInt.value limbBits input_b) % P381))[i.val] := by
    intro i
    have hget := h_env.1 i
...
/-- The `SubMod` formal circuit: `c = a − b mod P381`. -/
