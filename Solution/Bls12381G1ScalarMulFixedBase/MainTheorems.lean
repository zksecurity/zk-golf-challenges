...
`ScalarMul` gadget (6×64-bit little-endian limbs) at the **fixed** base point
...
open Challenge.Instances.Bls12381G1ScalarMulFixedBase
...
/-- The generator's x-coordinate as a natural number (the IETF/Zcash BLS12-381
G1 generator `G.x`). -/
def gxNat : ℕ := 0x17f1d3a73197d7942695638c4fa9ac0fc3688c4f9774b905a14e3a3f171bac586c55e83ff97a1aeffb3af00adb22c6bb

/-- The generator's y-coordinate as a natural number (the IETF/Zcash BLS12-381
G1 generator `G.y`). -/
def gyNat : ℕ := 0x8b3f481e3aaa0f1a09e30ed741d8ae4fcf5e095d5d00af600db18cb2c04b3edd03cc744a2888ae40caa232946c5e7e1
...
lemma gxNat_cast : ((gxNat : ℕ) : Specs.Bls12381G1.Fp) = Specs.Bls12381G1.G.x := by rfl

/-- `gyNat`, as an element of the base field, is `G.y`. -/
lemma gyNat_cast : ((gyNat : ℕ) : Specs.Bls12381G1.Fp) = Specs.Bls12381G1.G.y := by rfl
...
lemma decodeFe_gxConst (env : Environment (F circomPrime)) :
    decodeFe (Vector.map (Expression.eval env) gxConst) = Specs.Bls12381G1.G.x := by
...
lemma decodeFe_gyConst (env : Environment (F circomPrime)) :
    decodeFe (Vector.map (Expression.eval env) gyConst) = Specs.Bls12381G1.G.y := by
...
never `native_decide`). -/
lemma G_onCurve :
    Specs.ShortWeierstrass.OnCurve Specs.Bls12381G1.curve Specs.Bls12381G1.G := by
  have key : gyNat ^ 2 % Specs.Bls12381G1.p
      = (gxNat ^ 3 + 4) % Specs.Bls12381G1.p := by decide
  show Specs.Bls12381G1.G.y ^ 2
    = Specs.Bls12381G1.G.x ^ 3 + Specs.Bls12381G1.curve.a * Specs.Bls12381G1.G.x
        + Specs.Bls12381G1.curve.b
  rw [show Specs.Bls12381G1.curve.a = 0 from rfl,
      show Specs.Bls12381G1.curve.b = (4 : Specs.Bls12381G1.Fp) from rfl,
      zero_mul, add_zero, ← gxNat_cast, ← gyNat_cast]
  have hcast : ((gyNat ^ 2 : ℕ) : Specs.Bls12381G1.Fp)
      = ((gxNat ^ 3 + 4 : ℕ) : Specs.Bls12381G1.Fp) := by
