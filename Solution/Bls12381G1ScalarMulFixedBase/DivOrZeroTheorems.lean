...
- `decodeFe` bridges between canonical big-integer values and the emulated
  field, and the two arithmetic cores certifying `λ · den ≡ num (mod P381)`.
...
lemma one_lt_P381 : 1 < P381 := by decide
...
instance : NeZero P381 := ⟨Nat.pos_iff_ne_zero.mp P381_pos⟩
...
lemma eval_pConst (env : Environment (F circomPrime)) :
    Vector.map (Expression.eval env) pConst = emuOfNat P381 :=
  eval_emuConst env P381
...
`l · d ≡ n (mod P381)` on naturals descends to the emulated field. -/
lemma mul_cast_of_mod_eq {l d n : ℕ} (h : l * d % P381 = n) :
    (l : Specs.Bls12381G1.Fp) * (d : Specs.Bls12381G1.Fp) = (n : Specs.Bls12381G1.Fp) := by
...
lemma witness_val_den_zero (n : Specs.Bls12381G1.Fp) :
    (n * (0 : Specs.Bls12381G1.Fp)⁻¹).val = 0 := by
  rw [inv_zero, mul_zero, ZMod.val_zero]

/-- **Completeness core, nonzero case.** The canonical value of the witnessed
quotient `n · d⁻¹` certifies the multiplication: `⌜n · d⁻¹⌝ · d ≡ n (mod P381)`
for canonical `n` and a nonzero denominator. -/
lemma witness_cert_nonzero {nv dv : ℕ} (hnv : nv < P381)
    (hd : (dv : Specs.Bls12381G1.Fp) ≠ 0) :
    ((nv : Specs.Bls12381G1.Fp) * (dv : Specs.Bls12381G1.Fp)⁻¹).val * dv % P381 = nv := by
  have h1 : ((((nv : Specs.Bls12381G1.Fp) * (dv : Specs.Bls12381G1.Fp)⁻¹).val * dv : ℕ) :
      Specs.Bls12381G1.Fp) = (nv : Specs.Bls12381G1.Fp) := by
    push_cast [ZMod.natCast_val, ZMod.cast_id]
    rw [mul_assoc, inv_mul_cancel₀ hd, mul_one]
  calc ((nv : Specs.Bls12381G1.Fp) * (dv : Specs.Bls12381G1.Fp)⁻¹).val * dv % P381
      = ((((nv : Specs.Bls12381G1.Fp) * (dv : Specs.Bls12381G1.Fp)⁻¹).val * dv : ℕ) :
          Specs.Bls12381G1.Fp).val := by rw [ZMod.val_natCast]
    _ = ((nv : Specs.Bls12381G1.Fp)).val := by rw [h1]
