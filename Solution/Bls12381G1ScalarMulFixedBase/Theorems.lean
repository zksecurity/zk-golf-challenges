...
namespace Solution.Bls12381G1ScalarMulFixedBase.Limbs
...
end Solution.Bls12381G1ScalarMulFixedBase.Limbs
...
spec via `Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs`. -/
def value (B : ℕ) (x : BigInt m (F p)) : ℕ :=
  Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B (x.toList.map ZMod.val)
...
theorem fromLimbs_injective {B : ℕ} :
    ∀ {l₁ l₂ : List ℕ}, l₁.length = l₂.length →
      (∀ x ∈ l₁, x < 2 ^ B) → (∀ x ∈ l₂, x < 2 ^ B) →
      Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B l₁ = Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B l₂ → l₁ = l₂ := by
  intro l₁
  induction l₁ with
  | nil =>
    intro l₂ hlen _ _ _
  ...
  | cons a t ih =>
    ...
    | b :: s =>
      ...
      simp only [Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs, List.foldr_cons] at hval ⊢
...
theorem fromLimbs_eq_sum {B : ℕ} (l : List ℕ) :
    Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B l = ∑ i : Fin l.length, l[i] * 2 ^ (B * i.val) := by
  induction l with
  | nil => simp [Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs]
  | cons a t ih =>
    have hrhs : (∑ i : Fin (a :: t).length, (a :: t)[i] * 2 ^ (B * i.val))
      ...
      ring
    rw [show Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B (a :: t)
        = a + Solution.Bls12381G1ScalarMulFixedBase.Limbs.fromLimbs B t * 2 ^ B from rfl, ih]
    exact hrhs.symm
