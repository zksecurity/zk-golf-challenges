...
import Solution.Bls12381G1ScalarMul.MainTheorems
import Solution.Bls12381G1ScalarMul.Cost
...
the `ScalarMul` gadget (6×64-bit little-endian limbs) and exports the
...
@[reducible] def allocations : Nat := 12521205
@[reducible] def constraints : Nat := 12663507
...
open Solution.Bls12381G1ScalarMul.Cost
...
by the witness IR, never by a Lean closure. One `ScalarMul` subcircuit;
`packCoord` is a pure reindexing that contributes no operations. -/
...
theorem computableWitness : ∀ n input,
  ...
  have hstruct :
    ...
    · exact eval_packCoord_congr input.px (eval_input_parts_eq h_input_eq).2.1
    · exact eval_packCoord_congr input.py (eval_input_parts_eq h_input_eq).2.2
