import Solution.Bls12381G1ScalarMul.MulModLTheorems
import Solution.Bls12381G1ScalarMul.AddModTheorems

/-!
# Emulated field multiplication, *lazy* variant — `MulModL`

`FormalCircuit` computing `c ≡ a · b (mod P381)` with the modulus fixed to the
...
The first operand `a` must be canonical (`< P381`) so the honest quotient
`q = a·b / P381 < 2^256` fits in `numLimbs` limbs; the second operand `b` may be
...
The quotient and the remainder of `a.value · b.value` by `P381`, from `MulMod`'s digit
...
def circuit : FormalCircuit (F circomPrime) Inputs Emu where
  ...
  soundness := by
    ...
    refine ⟨hrn, ?_⟩
    rw [show BigInt.value secpParams.B (Vector.map (Expression.eval env) pConst) = P381 from
      pConst_value env] at hid
    have hcast := congrArg (Nat.cast : ℕ → Specs.Bls12381G1.Fp) hid
    push_cast at hcast
  ...
  completeness := by
    ...
    have hpv : BigInt.value secpParams.B (Vector.map (Expression.eval env.toEnvironment) pConst) = P381 :=
    ...
    have core := MulMod.mulMod_completeness_core_wm_loose (B := secpParams.B) secpParams.hB hp i₀
      ...
      (by rw [hpv]; exact ha_valid.2) (by rw [hpv]; exact P381_pos)
