...
lemma output_spec
    ...
    (hmx : Vector.map (Expression.eval env)
      ...
      then Vector.map (Expression.eval env) zb
        ...
        (varFromOffset (fields coordBytes) (i₀ + 12520245)))
    (hbits : Specs.ShortWeierstrass.IsBitArray (Vector.map ZMod.val input_bits))
    (honcurve : Specs.ShortWeierstrass.OnCurve Specs.Bls12381G1.curve
      { x := decodeFe input_px, y := decodeFe input_py })
    (hsub : Specs.Bls12381G1.InSubgroup (.affine
      { x := decodeFe input_px, y := decodeFe input_py }))
    (hmy : Vector.map (Expression.eval env)
        (varFromOffset (fields coordBytes) (i₀ + 12520245 + 432 + 432 + 48)) =
      if Expression.eval env (accVar i₀ 255).isInf = 1
      then Vector.map (Expression.eval env) zb
        ...
        (varFromOffset (fields coordBytes) (i₀ + 12520245 + 432))) :
    Specs.Bls12381G1ScalarMul.Spec (Vector.map ZMod.val input_bits)
      { x := decodeFe input_px, y := decodeFe input_py }
      (decodeOutput
        { x := Vector.map (Expression.eval env)
            (Vector.ofFn fun i =>
  ...
  unfold Specs.Bls12381G1ScalarMul.Spec Specs.ShortWeierstrass.Spec
    decodeOutput
    ...
    · rw [hinf1] at hdec256 ⊢
      ...
      rw [show specAcc (Vector.map ZMod.val input_bits)
          ...
          specAcc (Vector.map ZMod.val input_bits)
            { x := decodeFe input_px, y := decodeFe input_py } 255 from rfl]
      exact hdec256.symm
  · -- the decoded output lies in the prime-order subgroup: it is
    -- `[scalarOfBits bits]P` with `P` in the subgroup (closed under the
    -- group law), via `inSubgroup_nsmul`
    rcases hbool with hinf0 | hinf1
    · rw [hinf0] at hdec256 ⊢
      rw [if_neg (zero_ne_one (α := F circomPrime))] at hdec256
      rw [if_neg (zero_ne_one (α := F circomPrime)),
        if_neg (zero_ne_one (α := F circomPrime)),
        if_neg (zero_ne_one (α := F circomPrime))]
      simp only [coordVal, fromLimbs_rev_ofFn, hxb_val, hyb_val, decodeFe] at hdec256 ⊢
      rw [hdec256, ← scalarMul_eq_specAcc,
        Specs.Bls12381G1.scalarMul_eq_nsmul _ hbits _]
      exact Specs.Bls12381G1.inSubgroup_nsmul honcurve hsub
    · rw [hinf1] at hdec256 ⊢
      rw [if_pos rfl] at hdec256
      rw [if_pos rfl]
      rfl
