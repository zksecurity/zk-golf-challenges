...
`ScalarMul` gadget (6×64-bit little-endian limbs, `Limbs.fromLimbs`). This file
...
open Challenge.Instances.Bls12381G1ScalarMul
open Solution.Bls12381G1ScalarMul.Limbs

/-- Every truncated-subtraction index `47 − j` hits the 48-byte coordinate. -/
theorem idx_lt (j : ℕ) : 47 - j < Interface.coordBytes := by
...
big-endian byte `47 − j`. -/
theorem coordVal_eq_le_sum (v : Vector (F circomPrime) Interface.coordBytes) :
    Interface.coordVal v
      = ∑ j ∈ Finset.range 48, (v[47 - j]'(idx_lt j)).val * 2 ^ (8 * j) := by
  rw [coordVal_eq, ScalarMul.coordVal, fromLimbs_eq_sum,
    ← Fin.sum_univ_eq_sum_range (fun j => (v[47 - j]'(idx_lt j)).val * 2 ^ (8 * j)) 48]
  have hlen : ((v.toList.map ZMod.val).reverse).length = 48 := by simp
  rw [← Fin.sum_congr'
    (fun j : Fin 48 => (v[47 - j.val]'(idx_lt j.val)).val * 2 ^ (8 * j.val)) hlen]
...
`47 − (8k+t)` with weight `2^(8t)` — exactly `eval env ∘ Main.packCoord`. -/
...
theorem pack_getElem_val (b : Vector (F circomPrime) Interface.coordBytes)
    ...
    (k : ℕ) (hk : k < numLimbs) :
    ((pack b)[k]'hk).val
      = ∑ t ∈ Finset.range 8, (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t) := by
  have hcast : (pack b)[k]'hk
      = ((∑ t ∈ Finset.range 8,
          (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t) : ℕ) : F circomPrime) := by
    rw [pack, Vector.getElem_ofFn,
      ← Fin.sum_univ_eq_sum_range
        (fun t => (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t)) 8,
      Nat.cast_sum]
    apply Finset.sum_congr rfl
    intro t _
  ...
  have hlt : (∑ t ∈ Finset.range 8,
      (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t)) < 2 ^ 64 := by
    have h := sum_lt_pow (B := 8) (n := 8)
      (fun t : Fin 8 => (b[47 - (8 * k + t.val)]'(idx_lt _)).val)
      (fun t => hb ⟨47 - (8 * k + t.val), idx_lt _⟩)
    rw [← Fin.sum_univ_eq_sum_range
      (fun t => (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t)) 8]
    calc (∑ t : Fin 8, (b[47 - (8 * k + t.val)]'(idx_lt _)).val * 2 ^ (8 * t.val))
        < 2 ^ (8 * 8) := h
      _ = 2 ^ 64 := by norm_num
...
theorem pack_normalized (b : Vector (F circomPrime) Interface.coordBytes)
  ...
  have h := sum_lt_pow (B := 8) (n := 8)
    (fun t : Fin 8 => (b[47 - (8 * k.val + t.val)]'(idx_lt _)).val)
    (fun t => hb ⟨47 - (8 * k.val + t.val), idx_lt _⟩)
  rw [← Fin.sum_univ_eq_sum_range
    (fun t => (b[47 - (8 * k.val + t)]'(idx_lt _)).val * 2 ^ (8 * t)) 8]
  calc (∑ t : Fin 8, (b[47 - (8 * k.val + t.val)]'(idx_lt _)).val * 2 ^ (8 * t.val))
      < 2 ^ (8 * 8) := h
...
theorem pack_value (b : Vector (F circomPrime) Interface.coordBytes)
  ...
  calc (∑ k : Fin numLimbs, ((pack b)[k]).val * 2 ^ (limbBits * k.val))
      = ∑ k ∈ Finset.range 6,
          (∑ t ∈ Finset.range 8, (b[47 - (8 * k + t)]'(idx_lt _)).val * 2 ^ (8 * t))
            * 2 ^ (8 * 8 * k) := by
        ...
        intro k _
        rw [Fin.getElem_fin, pack_getElem_val b hb k.val k.isLt]
    _ = ∑ j ∈ Finset.range (6 * 8), (b[47 - j]'(idx_lt _)).val * 2 ^ (8 * j) :=
        (ToBytes.sum_regroup 8 8 (fun j => (b[47 - j]'(idx_lt _)).val) 6).symm
    _ = ∑ j ∈ Finset.range 48, (b[47 - j]'(idx_lt _)).val * 2 ^ (8 * j) := by norm_num
...
theorem pack_valid (b : Vector (F circomPrime) Interface.coordBytes)
    (hb : ∀ i : Fin Interface.coordBytes, (b[i]).val < 256)
    (hlt : Interface.coordVal b < Specs.Bls12381G1.p) :
    Fe.Valid (pack b) :=
...
lemma eval_input_parts_eq {input : Var Interface.Input (F Interface.circomPrime)}
    ...
    Vector.map (Expression.eval e1.toEnvironment) input.bits
        = Vector.map (Expression.eval e2.toEnvironment) input.bits ∧
      Vector.map (Expression.eval e1.toEnvironment) input.px
        = Vector.map (Expression.eval e2.toEnvironment) input.px ∧
      Vector.map (Expression.eval e1.toEnvironment) input.py
        = Vector.map (Expression.eval e2.toEnvironment) input.py := by
