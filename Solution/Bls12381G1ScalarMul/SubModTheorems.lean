import Solution.Bls12381G1ScalarMul.AddModTheorems
...
difference `r = (a + P381 − b) % P381` satisfies `r + b = a + q·P381` with the
borrow bit `q = [a < b]`. -/
lemma sub_witness_identity {va vb : ℕ} (hva : va < P381) (hvb : vb < P381) :
    (va + P381 - vb) % P381 + vb = va + (if va < vb then 1 else 0) * P381 := by
  by_cases h : va < vb
  · rw [if_pos h, Nat.mod_eq_of_lt (by omega : va + P381 - vb < P381)]
    omega
  · rw [if_neg h, show va + P381 - vb = (va - vb) + P381 from by omega,
      Nat.add_mod_right, Nat.mod_eq_of_lt (by omega : va - vb < P381)]
    omega
...
lemma rhs_bounds (env : Environment (F circomPrime)) (i₀ : ℕ)
  ...
  · rw [dif_pos hk,
    ...
    have h3 := limbOfNat_lt P381 k.val
    have h4 : qN * limbOfNat P381 k.val ≤ 1 * limbOfNat P381 k.val :=
...
lemma polyValue_rhs (env : Environment (F circomPrime)) (i₀ : ℕ)
  ...
  · -- sum evaluation
    ...
    have hsplit : (∑ k ∈ Finset.range numLimbs,
          ((if h : k < numLimbs then (a[k]'h).val else 0) + qN * limbOfNat P381 k)
            * 2 ^ (limbBits * k))
        = (∑ k ∈ Finset.range numLimbs,
            (if h : k < numLimbs then (a[k]'h).val else 0) * 2 ^ (limbBits * k))
          + ∑ k ∈ Finset.range numLimbs, qN * (limbOfNat P381 k * 2 ^ (limbBits * k)) := by
...
lemma soundness_core (i₀ : ℕ) (env : Environment (F circomPrime))
  ...
  -- push the ℕ identity `r + b = a + q·P381` into the emulated field
...
/-- **Completeness core.** The honest witnesses (`r = (a + P381 − b) % P381`
...
lemma completeness_core (i₀ : ℕ) (env : Environment (F circomPrime))
    ...
    (h_wit_r : ∀ i : Fin numLimbs,
      env.get (i₀ + i.val)
        = (emuOfNat ((BigInt.value limbBits a + P381 - BigInt.value limbBits b) % P381))[i.val])
  ...
  set rN := (va + P381 - vb) % P381 with hrN
  ...
  have hkey : rN + vb = va + qNat * P381 := by
    rw [hrN, hqNat]
    exact sub_witness_identity ha_lt hb_lt
  -- the witnessed output limbs are the canonical digits of (a + P381 − b) % P381
  ...
  · -- the EqViaCarries identity: r + b = a + q·P381 over ℕ
