import Solution.Bls12381G1ScalarMul.MulMod
import Solution.Bls12381G1ScalarMul.WitgenLimbs
import Challenge.Specs.Bls12381G1
import Challenge.Instances.Bls12381G1ScalarMul.Interface
...
bridge circuit values to the trusted spec (`Challenge.Specs.Bls12381G1`).
...
@[reducible] def circomPrime : ℕ :=
  Challenge.Instances.Bls12381G1ScalarMul.Interface.circomPrime

instance : Fact (circomPrime > 2) := ⟨by decide⟩

/-- The emulated secp256k1 base-field prime (`2^256 - 2^32 - 977`). -/
@[reducible] def P381 : ℕ := Specs.Bls12381G1.p

/-- Number of limbs of an emulated BLS12-381 base-field element. -/
@[reducible] def numLimbs : ℕ := 6
...
(covering exactly 256 bits; canonical values are `< P381 < 2^256`). -/
@[reducible] def Emu : TypeMap := BigInt numLimbs

/-- Big-integer parameters for 381-bit values over the circom prime: 6 limbs
...
def pConst : Var Emu (F circomPrime) := emuConst P381
...
Every emulated-arithmetic gadget (`AddMod`, `SubMod`, `DivOrZero`, `ToBytes`) works with the same two shapes: the base-`2^limbBits` **value**
...
Reading an element in costs nothing: its six limbs are disjoint 64-bit windows of the
...
complement, which existed only because a 384-bit value cannot be routed through the
field, is gone.
-/
...
/-- Digits of an emulated element: `limbBits · numLimbs = 384` bits. -/
@[reducible] def emuLen : ℕ := numChunks (limbBits * numLimbs)

/-- The width the limb decomposition covers: `2^(limbBits·numLimbs) = 2^384`. -/
...
Both emulated add/subtract gadgets reduce to the same two moves: build a
`workLen`-digit register holding `a + b` or `a + C − b` for a constant shift `C`, then
conditionally subtract the modulus once. (The secp256k1 solution also carries lazy
`AddModL`/`SubModL` variants that reduce twice; over the 381-bit prime a 6-limb
normalized operand can exceed `2^384 / P381 ≈ 19` moduli, so two conditional
subtractions do not cover them and those variants are omitted here.)

`reduceStep` is that conditional subtraction: it subtracts `P381` and keeps the
...
def redN (L X : ℕ) : ℕ :=
  if P381 % base ^ L ≤ X % base ^ L then X % base ^ L - P381 % base ^ L else X % base ^ L

/-- Borrow of one reduction stage: `1` exactly when no reduction happened. -/
def redB (L X : ℕ) : ℕ := if P381 % base ^ L ≤ X % base ^ L then 0 else 1

/-- One conditional reduction by the modulus: subtract `P381` when that keeps the
...
def reduceStep (L : ℕ) (x : List (U64Expr (F circomPrime))) :
  ...
  let d ← subP x (constDigits P381 L) (uc 0)
...
theorem computes_reduceStep {S : Array (Step (F circomPrime))} {L : ℕ}
  ...
  refine Computes.bind (computesBigU_subP _ _ (uc 0) hx (evalsBig_constDigits P381 L)
    (EvalsU.uc 0 (by norm_num)) (fun _ => by norm_num)) ?_
  intro S1 d hS1 hd
  have hd1 : EvalsBig S1 d.1
      (fun env => subb (ofNat (X env) L) (ofNat P381 L) 0) := hd.1
  have hd2 : EvalsU S1 d.2
      (fun env => subbOut (ofNat (X env) L) (ofNat P381 L) 0) := hd.2
  ...
  · have hbd : Bounded (if subbOut (ofNat (X env) L) (ofNat P381 L) 0 = 0
        then subb (ofNat (X env) L) (ofNat P381 L) 0 else ofNat (X env) L) := by
      split
    ...
    · rw [redN]
      by_cases hle : P381 % base ^ L ≤ X env % base ^ L
      · rw [if_pos (by
      ...
      · rw [if_neg (by
          intro hz
          exact hle (by
            have := (subbOut_eq_zero_iff (bounded_ofNat (X env) L) (bounded_ofNat P381 L)
  ...
  · simp only [redB]
    ...
    · rw [if_pos hle]
      exact (subbOut_eq_zero_iff (bounded_ofNat _ _) (bounded_ofNat _ _)
        (by rw [length_ofNat, length_ofNat])).mpr (by rw [lval_ofNat, lval_ofNat]; exact hle)
    · rw [if_neg hle]
      have hb := subbOut_lt (ofNat (X env) L) (ofNat P381 L) 0 (by norm_num)
      rcases (show subbOut (ofNat (X env) L) (ofNat P381 L) 0 = 0
          ∨ subbOut (ofNat (X env) L) (ofNat P381 L) 0 = 1 by omega) with h | h
      · exfalso
        exact hle (by
          have := (subbOut_eq_zero_iff (bounded_ofNat (X env) L) (bounded_ofNat P381 L)
...
/-- Digits of the working register: `2·P381` plus a 384-bit value still fits. -/
@[reducible] def workLen : ℕ := emuLen + 1

theorem span_lt_work : emuSpan < base ^ workLen := by decide

theorem modulus_lt_work : P381 < base ^ workLen := by decide
...
`2·P381`, which is a case split rather than a division.
-/
...
theorem two_mul_modulus_lt_work : 2 * P381 < base ^ workLen := by decide

theorem redN_eq {X : ℕ} (h : X < 2 * P381) : redN workLen X = X % P381 := by
  ...
  rw [redN, Nat.mod_eq_of_lt modulus_lt_work, Nat.mod_eq_of_lt hX,
    (mod_div_two_mul (by decide : 0 < P381) h).1]

theorem redB_eq {X : ℕ} (h : X < 2 * P381) :
    (if redB workLen X = 0 then 1 else 0) = X / P381 := by
  have hw := two_mul_modulus_lt_work
  have hX : X < base ^ workLen := by omega
  rw [redB, Nat.mod_eq_of_lt modulus_lt_work, Nat.mod_eq_of_lt hX,
    (mod_div_two_mul (by decide : 0 < P381) h).2]
  split <;> simp
...
/-- `r = a + q·P381 − b` with `q` the single borrow: the canonical-operand difference. -/
def subRProg (a b : Var Emu (F circomPrime)) :
    M (F circomPrime) (VExpr (F circomPrime) numLimbs) := do
  let s ← shiftSub P381 a b
  let rd ← reduceStep workLen s
  Pure.pure (emuOutF rd.1)
...
/-! ### What the four programs compute -/
...
theorem computesV_subRProg (a b : Var Emu (F circomPrime)) :
    ComputesV #[] (subRProg a b)
      (fun env => emuOfNat
        (redN workLen (shiftN P381 (bigVal limbBits a env) (bigVal limbBits b env)))) := by
  refine Computes.bind (computesBig_shiftSub P381 a b) ?_
  intro S1 s hS1 hs
  refine Computes.bind (computes_reduceStep hs) ?_
  intro S2 rd hS2 hrd
  have h1 : EvalsBig S2 rd.1 (fun env =>
      ofNat (redN workLen (shiftN P381 (bigVal limbBits a env) (bigVal limbBits b env)))
        workLen) := hrd.1
  exact Computes.pure ((evalsV_emuOutF h1).congr fun env => by
    rw [lval_ofNat_of_lt (redN_lt _ _)])
...
theorem computesF_subQProg (a b : Var Emu (F circomPrime)) :
    ComputesF #[] (subQProg a b)
      (fun env => FiniteField.fromNat
        (redB workLen (shiftN P381 (bigVal limbBits a env) (bigVal limbBits b env)))) := by
  refine Computes.bind (computesBig_shiftSub P381 a b) ?_
  intro S1 s hS1 hs
  refine Computes.bind (computes_reduceStep hs) ?_
  intro S2 rd hS2 hrd
  have h2 : EvalsU S2 rd.2 (fun env =>
      redB workLen (shiftN P381 (bigVal limbBits a env) (bigVal limbBits b env))) := hrd.2
...
theorem eval_addRProg_of (a b : Var Emu (F circomPrime))
    ...
    (hna : BigInt.Normalized limbBits (a.map (Expression.eval env.toEnvironment)))
    (hnb : BigInt.Normalized limbBits (b.map (Expression.eval env.toEnvironment)))
    (hva : evalEmu env a < P381) (hvb : evalEmu env b < P381) :
    Witgen.VExpr.eval
        { env := env, locals := Witgen.evalSteps env (addRProg a b #[]).2.toList }
        (addRProg a b #[]).1
      = emuOfNat ((evalEmu env a + evalEmu env b) % P381) := by
...
theorem eval_addQProg_of (a b : Var Emu (F circomPrime))
    ...
    Witgen.FExpr.eval
        ...
        (addQProg a b #[]).1
      = (((evalEmu env a + evalEmu env b) / P381 : ℕ) : F circomPrime) := by
  obtain ⟨ha, hb⟩ := bigVal_pair hna hnb
...
theorem eval_subRProg_of (a b : Var Emu (F circomPrime))
    ...
    Witgen.VExpr.eval
        ...
        (subRProg a b #[]).1
      = emuOfNat ((evalEmu env a + P381 - evalEmu env b) % P381) := by
  obtain ⟨ha, hb⟩ := bigVal_pair hna hnb
  have hs : shiftN P381 (bigVal limbBits a env) (bigVal limbBits b env)
      = evalEmu env a + P381 - evalEmu env b := by
    rw [ha, hb]
...
theorem eval_subQProg_of (a b : Var Emu (F circomPrime))
  ...
  rw [eval_programF (computesF_subQProg a b) env, hs, redB, FiniteField.fromNat_F,
    ...
    Nat.mod_eq_of_lt (show evalEmu env a + P381 - evalEmu env b < base ^ workLen by
...
The four programs are sealed once their specifications are proved. A witness program
...
attribute [irreducible]
  addRProg addQProg subRProg subQProg
...
spec. -/
def decodeFe (x : Emu (F circomPrime)) : Specs.Bls12381G1.Fp :=
  ((x.value limbBits : ℕ) : Specs.Bls12381G1.Fp)

/-- A well-formed emulated field element: normalized limbs and a canonical
(`< P381`) value. On canonical elements `decodeFe` is injective
(`BigInt.value_inj`), so spec-level equality is equivalent to limb-wise
equality. -/
def Fe.Valid (x : Emu (F circomPrime)) : Prop :=
  x.Normalized limbBits ∧ x.value limbBits < P381
