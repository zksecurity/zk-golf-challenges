...
* the modular-exponentiation bit helpers (`eBits`, `ofBits`) and the constant-`1`
  big integer `oneVal`;
...
spec via `Specs.RSA.fromLimbs`. -/
def value (B : ℕ) (x : BigInt m (F p)) : ℕ :=
  Specs.RSA.fromLimbs B (x.toList.map ZMod.val)
...
theorem fromLimbs_injective {B : ℕ} :
    ∀ {l₁ l₂ : List ℕ}, l₁.length = l₂.length →
      (∀ x ∈ l₁, x < 2 ^ B) → (∀ x ∈ l₂, x < 2 ^ B) →
      Specs.RSA.fromLimbs B l₁ = Specs.RSA.fromLimbs B l₂ → l₁ = l₂ := by
  intro l₁
  induction l₁ with
  | nil =>
    intro l₂ hlen _ _ _
  ...
  | cons a t ih =>
    ...
    | b :: s =>
      ...
      simp only [Specs.RSA.fromLimbs, List.foldr_cons] at hval ⊢
...
theorem fromLimbs_eq_sum {B : ℕ} (l : List ℕ) :
    Specs.RSA.fromLimbs B l = ∑ i : Fin l.length, l[i] * 2 ^ (B * i.val) := by
  induction l with
  | nil => simp [Specs.RSA.fromLimbs]
  | cons a t ih =>
    have hrhs : (∑ i : Fin (a :: t).length, (a :: t)[i] * 2 ^ (B * i.val))
      ...
      ring
    rw [show Specs.RSA.fromLimbs B (a :: t)
        = a + Specs.RSA.fromLimbs B t * 2 ^ B from rfl, ih]
    exact hrhs.symm
...
/-- Bundled parameters for the RSA gadgets (`ModExp`, `RSARecover`, `RSAVerify`):
the big-integer parameters (`bigIntParams`) together with the compile-time public
exponent `e`. The big-integer parameters are a genuine field, passed down to the
big-integer gadgets as `P.bigIntParams`. -/
structure RSAParams (p m : ℕ) where
  /-- the big-integer parameters (limb/carry widths and field-size hypotheses) -/
  bigIntParams : BigIntParams p m
  /-- compile-time public (verification) exponent -/
  e : ℕ
