...
(`soundness`, `completeness`, `mainCost`, `isR1CS`, `witgenIsIR`). All supporting material
...
instance elaborated : ElaboratedCircuit (F circomPrime) Input Output main where
  ...
  channelsLawful := by
    ...
    simp only [main, circuit_norm, BytesToBigInt.circuit, BytesToBigInt.elaborated,
      PadDigest.circuit, PadDigest.elaborated, LessThan.circuit, LessThan.elaborated,
      Equal.circuit, Equal.elaborated, hMEg]
...
/-- Every witness in the circuit — the three byte/digest bit vectors, and everything
inside `LessThan` / `ModExp` (hence `MulMod`, `Normalize`, `EqViaCarries`) — is
generated by the witness IR, never by a Lean closure. Assembled by structural
recursion over `main`'s `do`-block, exactly like `costIs_main`. -/
theorem witgenIsIR : Challenge.WitgenIR.witgenIsIR main := fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_sub_bytesToBigInt _) fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_sub_bytesToBigInt _) fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_sub_padDigest _) fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_assertion_lessThan bigIntParams4096 _) fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_sub_modExp params4096 _) fun _ =>
  Challenge.WitgenIR.UsesIRCirc.bind (usesIR_assertion_equal bigIntParams4096 _) fun _ =>
