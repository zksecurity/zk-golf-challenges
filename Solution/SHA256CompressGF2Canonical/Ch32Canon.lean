...
/-!
# `Ch32Canon`: canonical (C = identity) Ch gadget (+ shared `zxorOut`/`wv_getElem`)
-/
...
open Solution.SHA256CompressGF2 (a96 a96_affine ch_words)

/-- Witness the AND `pᵢ = eᵢ·(fᵢ⊕gᵢ)`, pin with `pᵢ − eᵢ·(fᵢ⊕gᵢ) = 0` (C = pᵢ),
and output the inlined `gᵢ ⊕ pᵢ = Ch`. -/
...
/-- The canonical `Ch` output is the input word `g` xored with the AND witnesses,
so agreement needs the input as well as the block. -/
