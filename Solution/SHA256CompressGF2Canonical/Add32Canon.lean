...
Same ripple-carry adder as `Solution.SHA256CompressGF2.Add32`, but every
constraint row's `C`-side is a single fresh witness variable (identity `C`).
...
Cost per adder: 62 witnesses / 62 constraints (2× the non-canonical `Add32`).
-/
...
open Solution.SHA256CompressGF2 (eval_fields_of_getElem eval_getElem_congr
...
`carryVal`, so the function-level `Add32.eval_at32_fun_congr` does not suffice. -/
